...
public final class Command {
    ...
    static String msg = "";
    public static TXCommand Error_f = () -> Error(Defines.ERR_FATAL, Cmd.Argv(1));
    /**
     * CRC table.
     */
    ...
    public static String Parse(ParseHelp hlp) {
        ...
        if (len == Defines.MAX_TOKEN_CHARS) {
            Command.Printf("Token exceeded " + Defines.MAX_TOKEN_CHARS + " chars, discarded.\n");
            len = 0;
        }
    ...
    public static void Error(int code, String fmt, Object ... vargs) throws QuakeException {
        // va_list argptr;
    ...
    public static void DPrintf(String fmt, Object ... vargs) {
        if (Context.developer == null || Context.developer.value == 0)
    ...
    public static void Printf(String fmt, Object ... vargs) {
        String msg = String.format(_debugContext + fmt, vargs);
        if (rd_target != 0) {
        ...
        if (Context.logfile_active != null && Context.logfile_active.value != 0) {
            ...
            if (Context.logfile == null) {
                ...
                if (Context.logfile_active.value > 2)
                    try {
                        ...
                        Context.logfile.seek(Context.logfile.length());
                    } catch (Exception e) {
            ...
            if (Context.logfile != null)
                try {
                    Context.logfile.writeChars(msg);
                } catch (IOException e) {
                    // TODO: do quake2 error handling!
                    e.printStackTrace();
                }
            if (Context.logfile_active.value > 1) ; // do nothing
    ...
    public static String sprintf(String fmt, Object ... vargs) {
        ...
        return String.format(fmt, vargs);
    }
