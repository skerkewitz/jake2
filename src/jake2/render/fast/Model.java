...
//	  ===============================================================================
    ...
    */
    ...
    /**
     * Lads in a model for the given name.
     */
    private TModel loadModelWithName(String name, boolean crash) {

        int i;

        if (name == null || name.length() == 0) {
            Command.Error(Defines.ERR_DROP, "loadModelWithName: NULL name");
        }
        ...
        if (name.charAt(0) == '*') {
            ...
            if (i < 1 || RenderAPIImpl.renderMain.r_worldmodel == null || i >= RenderAPIImpl.renderMain.r_worldmodel.numsubmodels)
	...
	*/
    ...
    private void Mod_LoadLighting(TLump l) {
        if (l.filelen == 0) {
    ...
    private void Mod_LoadVisibility(TLump l) {
        if (l.filelen == 0) {
            loadmodel.vis = null;
            return;
        }
    ...
    private void Mod_LoadVertexes(TLump l) {
    ...
    private void Mod_LoadSubmodels(TLump l) {
    ...
    private void Mod_LoadEdges(TLump l) {

        if ((l.filelen % TMEdge.DISK_SIZE) != 0)
            Command.Error(Defines.ERR_DROP, "MOD_LoadBmodel: funny lump size in " + loadmodel.name);

        int count = l.filelen / TMEdge.DISK_SIZE;
        // out = Hunk_Alloc ( (count + 1) * sizeof(*out));
        TMEdge[] edges = new TMEdge[count + 1];
    ...
    private void Mod_LoadTexinfo(TLump l) {
        TTexInfo in;
        TMapTexInfo[] out;
        TMapTexInfo step;
        int i, count;
        ...
        for (i = 0; i < count; i++) {
            ...
            if (out[i].image == null) {
                ...
                out[i].image = RenderAPIImpl.renderMain.r_notexture;
            }
        }
    ...
    ================
    */
    private void CalcSurfaceExtents(TMapSurface s) {
        float[] mins = {0, 0};
        float[] maxs = {0, 0};
        float val;
    ...
    private void Mod_LoadFaces(TLump l) {
        ...
        RenderAPIImpl.renderMain.currentmodel = loadmodel;
    ...
    private void Mod_LoadNodes(TLump l) {
        int i, j, count, p;
        qfiles.dnode_t in;
        TMNode[] out;
        ...
        for (i = 0; i < count; i++)
            out[i] = new TMNode(); // do first before linking
    ...
    private void Mod_LoadLeafs(TLump l) {
        qfiles.dleaf_t in;
        TMLeaf[] out;
        int i, j, count;
    ...
    private void Mod_LoadMarksurfaces(TLump l) {
        int i, j, count;
    ...
    private void Mod_LoadSurfedges(TLump l) {
        int i, count;
        int[] offsets;
    ...
    private void Mod_LoadPlanes(TLump l) {
        int i, j;
        cplane_t[] out;
        qfiles.dplane_t in;
        int count;
    ...
    private void Mod_LoadBrushModel(TModel mod, ByteBuffer buffer) {
        ...
        int j = header.version;
        if (j != Defines.BSPVERSION) {
            Command.Error(Defines.ERR_DROP, "Mod_LoadBrushModel: " + mod.name + " has wrong version number (" + j + " should be " + Defines.BSPVERSION + ")");
        }
	...
	*/
    ...
    private void Mod_LoadAliasModel(TModel mod, ByteBuffer buffer) {
        int i;
	...
	*/
    ...
    private void Mod_LoadSpriteModel(TModel mod, ByteBuffer buffer) {
...
//	  =============================================================================
    ...
    public void R_BeginRegistration(String model) {
        ...
        RenderAPIImpl.renderMain.registration_sequence++;
        RenderAPIImpl.renderMain.r_oldviewcluster = -1;        // force markleafs

        String fullname = "maps/" + model + ".bsp";

        // explicitly free the old map if different
        // this guarantees that mod_known[0] is the world map
        TVar flushmap = ConsoleVar.get("flushmap", "0", 0);
        if (!mod_known[0].name.equals(fullname) || flushmap.value != 0.0f) {
            Mod_Free(mod_known[0]);
        }

        RenderAPIImpl.renderMain.r_worldmodel = loadModelWithName(fullname, true);
        RenderAPIImpl.renderMain.r_viewcluster = -1;
    }
    ...
    */
    public TModel R_RegisterModel(String name) {
        ...
        mod = loadModelWithName(name, false);
        if (mod != null) {
            mod.registration_sequence = RenderAPIImpl.renderMain.registration_sequence;
            ...
            if (mod.type == mod_sprite) {
                ...
                for (i = 0; i < mod.numtexinfo; i++)
                    mod.texinfo[i].image.registration_sequence = RenderAPIImpl.renderMain.registration_sequence;
            }
        }
        return mod;
    }
    ...
    */
    public void R_EndRegistration() {
        ...
        for (int i = 0; i < mod_numknown; i++) {
            ...
            if (mod.name.length() == 0)
                continue;
            if (mod.registration_sequence != RenderAPIImpl.renderMain.registration_sequence) {    // don't need this model
                Mod_Free(mod);
            } else {
    ...
    private void Mod_Free(TModel mod) {
        mod.clear();
    }
